...
def collect_trajectories_policy_single(env, sac_agent, n=2000, state_indices=None, render=False, seed=None):
    ...
    for traj_no in range(n):
        ep_seed = rng.randint(0, 2**32-1) if seed is not None else None  #  1608637542  3421126067
...
def collect_trajectories_policy_single_re(env, sac_agent, n=2000, state_indices=None, render=False, seed=None):
    if seed is not None:
        rng = np.random.RandomState(seed)
