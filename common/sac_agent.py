...
class SquashedGaussianMLPActor(nn.Module):
    ...
    def forward(self, obs, deterministic=False, with_logprob=True):
        ...
        if deterministic:
            ...
            # pi_action = torch.clamp(mu + std * epsilon, -2, 2)
            # pi_action_max, pi_action_min = pi_action.max(), pi_action.min()


        if with_logprob:
            logpi_u = pi_distribution.log_prob(pi_action).sum(axis=-1) # compute log probability of action
            correct = (2*(np.log(2) - pi_action - F.softplus(-2*pi_action))).sum(axis=1)
            logp_pi = logpi_u - correct  # idk 
        else:
            logp_pi = None
        ...
        return pi_action, logp_pi, logpi_u.mean()
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, k, hidden_sizes=(512,512), add_time=False,
    ...
    def act(self, obs, deterministic=False, get_logprob=False):
        with torch.no_grad():
            a, logpi, _ = self.pi(obs, deterministic, True)
            if get_logprob:
